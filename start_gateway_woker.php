...
//        $socket->emit('login', "成功登陆");
        ...
        $socket->uid = $uid;
        // 更新这个socket对应页面的在线数据
        $socket->emit('update_online_count', "当前<b>{$last_online_count}</b>人在线，共打开<b>{$last_online_page_count}</b>个页面{$uid}");
    });
...
//                // 有指定uid则向uid所在socket组发送数据
//                if($to){
//                    $sender_io->to($to)->emit('new_notice', $_POST['content']);
//                // 否则向所有uid推送数据
//                }else{
//                    $sender_io->emit('new_notice', @$_POST['content']);
//                }
//                // http接口返回，如果用户离线socket返回fail
//                if($to && !isset($uidConnectionMap[$to])){
//                    return $http_connection->send('offline');
