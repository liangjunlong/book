...
class NotORM_Result extends NotORM_Abstract implements Iterator, ArrayAccess, Countable, JsonSerializable{
    ...
    function __construct($table, NotORM $notORM, $single = false){
        $this->table   = $table;
        $this->notORM  = $notORM; //lite对象 和PDO连接属性等参数
        $this->single  = $single;
        $this->primary = $notORM->structure->getPrimary($table);
	//	var_dump( $this->primary);exit;
    }
    ...
    function insert_multi(array $rows){
     ...
     //   echo "INSERT INTO $this->table $insert";exit;
	...
	//	echo "UPDATE" . $this->topString($this->limit) . " $this->table SET " . implode(", ", $values) . $this->whereString();
	//	echo '<br/>';
	...
	//	echo "DELETE" . $this->topString($this->limit) . " FROM $this->table" . $where;exit;
...
//		echo '<br/>';
    ...
    protected function execute(){
        ...
        if(!isset($this->rows)){
            ...
            //读取缓存
            $di = \PhalApi\DI();
            $index = md5($this->__toString().implode($parameters));
            $list = $di->cache->get($index);
            if(!$list) {
                //没有缓存，则从数据库读取
                try{
                ...
                $di->cache->set($index, $this->rows, COM_CACHE_TIMEOUT) ;
            }
            else {
                $this->rows = $list;
            }
	...
	function queryone($sql, $parmas = array()){
