...
class RedisCache implements Cache {
    ...
    public function __construct($config) {
		...
		$this->redis=new \RedisCluster(NULL,array($config['host'].':'.$config['port']));
//      // 连接
...
//      $this->prefix = isset($config['prefix']) ? $config['prefix'] : 'phalapi:';
    ...
    public function set($key, $value, $expire = 600) {
        $r=$this->redis->setex($this->formatKey($key), $expire, $this->formatValue($value));
		return $r;
    }
	...
	public function incr($key,$time){
		$num=$this->redis->incr($this->formatKey($key));
		$this->redis->expire($this->formatKey($key),$time);
		return $num; 
	}
