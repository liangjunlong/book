...
class CommonModel extends NotORM
{
	...
	public function begin($daname){
	//	$this->set_database($daname);
	//	\PhalApi\DI()->notorm->set_database($daname);
		return \PhalApi\DI()->notorm->beginTransaction($daname);
	}
	public function commit($daname){
	//	$this->set_database($daname);
	//	\PhalApi\DI()->notorm->set_database($daname);
		return \PhalApi\DI()->notorm->commit($daname);
	}
	public function rollback($daname){
		$this->set_database($daname);
		\PhalApi\DI()->notorm->set_database($daname);
		return \PhalApi\DI()->notorm->rollback($daname);
	}
	...
	public function set_database($db){
		...
		$all_dbname=$config->get('dbs')['servers'];
		foreach($all_dbname as $k=>$v){
			if($k==$db){
				$this->db=$v['name'];
				break;
			}
