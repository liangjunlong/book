...
class Group extends NotORM
{
    ...
    public function getGroupList($param)
    {
        ...
        $r = $this->getORM()->select($param['field'])->where('auth_group_title LIKE ?', "%" . $param['keyWord'] . "%")->or('`auth_group_desc` LIKE ?', "%" . $param['keyWord'] . "%")->limit($param['limitPage'], $param['limitCount'])->order($param['order'])->fetchAll();
        return $r;
    }
    ...
    {
        $r = $this->getORM()->where('auth_group_title LIKE ?', "%" . $keyWord . "%")->or('`auth_group_desc` LIKE ?', "%" . $keyWord . "%")->count();
        return $r;
    }
...
//            . ' WHERE g.uid = ' . $param['id'];
	 ...
	 */
	 ...
	 public function getalllist(){
	 	return $this->getORM()->select('id,auth_group_title')->where('id >0')->fetchRows();
	 }
    /**
     * 检测组名称是否重复
     * @param type $title
    ...
    {
        $r = $this->getORM()->select('id')->where('auth_group_title', $title)->where('id != ?', $id)->fetchOne();
    ...
    {
        $prefix = \PhalApi\DI()->config->get('dbs.tables.__default__.prefix');
        $sql = 'SELECT g.id, g.auth_group_rules, g.auth_group_menus  '
            ...
            . 'ON a.auth_group_access_group_id = g.id '
            . 'where a.auth_group_access_uid=:uid and g.auth_group_status=:status';
