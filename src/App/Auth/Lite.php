...
class Lite
{
    ...
    public function getMenuList($uid)
    {
        ...
        foreach ($groups as $g) {
            $ids = array_merge($ids, explode(',', trim($g['auth_group_menus'], ',')));
        }
    ...
    function formatList($menuList){
        ...
        foreach ($menuList as $item){
			...
			if($item['auth_rule_status']==0){
				continue;
			}
            if (isset($tmp[$item['auth_rule_pid']])){
                $tmp[$item['auth_rule_pid']]['nextItem'][] = &$tmp[$item['id']];
            }
            else{
                $list[] = &$tmp[$item['id']];
            }
    ...
    protected function getAuthList($uid)
    {
        ...
        $ids = array(); //保存用户所属组设置的所有权限规则id
        foreach ($groups as $g) {
            $ids = array_merge($ids, explode(',', trim($g['auth_group_rules'], ',')));
        }
        $ids = array_unique($ids);
        if (empty($ids)) {
            $_authList[$uid] = array();
        ...
        foreach ($rules as $rule) {
            if (!empty($rule['auth_rule_add_condition'])) { //根据addcondition进行验证
                $user = $userDomain->getUserInfo($uid); //获取用户信息,一维数组

                $command = preg_replace('/\{(\w*?)\}/', '$user[\'\\1\']', $rule['auth_rule_add_condition']);
                @(eval('$condition=(' . $command . ');'));
                if ($condition) {
                    $authList[] = strtolower($rule['auth_rule_name']);
                }
            } else {
                //只要存在就记录
                $authList[] = strtolower($rule['auth_rule_name']);
	...
	public function getAdminAuthList($uid){
        static $_authList = array(); //保存用户验证通过的权限列表
        //读取用户所属组
	
        $ruleDomain = new Domain\Rule();
        $rules = $ruleDomain->getAdminAuthList();
        //循环规则，判断结果。
        $authList = array();   //
        $userDomain = new Domain\User();
        foreach ($rules as $rule) {
	...
	//给获取到的权限排序
	public function order($authList){
		$data=array();
		$num=0;
		foreach($authList as $k=>$v){
			if($v['auth_rule_pid']<1){
				//排序
				//if($v['listorder'])
			}
		}
